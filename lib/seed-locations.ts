...
export async function seedLocations() {
  ...
  for (const stateCode of uniqueStates) {
    ...
    if (stateError) {
      console.error(`Error creating state ${stateName}:`, stateError);
      continue;
    }
  ...
  for (const [citySlug, countyName] of Object.entries(countyMapping)) {
    ...
    if (districtError) {
      console.error(`Error creating district ${countyName}:`, districtError);
      continue;
    }

    districtMap[citySlug] = district.id;
    console.log('Created district:', countyName);
  }
  ...
  for (const city of citiesData) {
    ...
    if (cityError) {
      console.error(`Error creating city ${city.name}:`, cityError);
      continue;
    }
